...
/* DBProv Protocol */
#define SQL_FIELD_NAME_MAX IMAGEMAGICK_FILE_NAME_LEN_MAX
struct provmsg_sql_read {
	struct provmsg msg;
  	uint32_t proc_id;
        int data_len;
	char data[0];
  /*
	int table_len;
	int column_len;
        char table_name[SQL_FIELD_NAME_MAX];
        char column_name[SQL_FIELD_NAME_MAX];
  */
} __attribute__((packed));

struct provmsg_sql_used {
	struct provmsg msg;
  	uint32_t proc_id;
	int table_len;
	int column_len;
...
#define LPM_UNIT_ID_MAX IMAGEMAGICK_FILE_NAME_LEN_MAX

struct provmsg_unit_start {
	struct provmsg msg;
	uuid_be uuid;
        char id[LPM_UNIT_ID_MAX];
} __attribute__((packed));

struct provmsg_unit_end {
	struct provmsg msg;
	uuid_be uuid;
} __attribute__((packed));
