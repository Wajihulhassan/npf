...
/* Bates - Helper functions for provenance */
#include "liblpm.h"
#include <sys/types.h>
#include <unistd.h>

#define LPM_PROVENANCE_UNITS 1
...
AP_CORE_DECLARE(int) ap_invoke_handler(request_rec *r)
{
    ...
    /* Bates -- Instrument "Unit Start" Message here! */
    char * uuid;
    if(LPM_PROVENANCE_UNITS)
	    uuid = lpm_unit_start(r->connection->remote_ip);

    /* Bates -- Handle the request */
    result = ap_run_handler(r);
    
    /* Bates -- Instrument "Unit End" Message here! */
    if(LPM_PROVENANCE_UNITS){
	    lpm_unit_end(uuid);
	    free(uuid);
    }
